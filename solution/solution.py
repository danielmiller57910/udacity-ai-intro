...
def naked_twins(values: dict) -> dict:
    # find all in grid with 2 options
    possible_twins = find_possible_twins(values.copy())
    # filter two those with 2 unique options
    shared_values = find_unique_twins(possible_twins)
    # filter to nodes that share an intersection
    peers = filter(nodes_share_intersection, shared_values)
    # map to add intersection of both twins and remove any duplicates and twins from map
    peers = map(identify_associated_nodes, peers)
    # iterate through every peer
    for peer in peers:
        value = peer['value']
        for intersection in peer['grid_intersection']:
            # delete shared value from every node in grid intersection
